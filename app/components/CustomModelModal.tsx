...
export default function CustomModelModal({ initial, onClose, onSave }: Props) {
  ...
  const [provider, setProvider] = useState<LlmProviderId>(initial?.provider || "qwen");
  const [model, setModel] = useState(initial?.model || "");
  const [baseUrl, setBaseUrl] = useState(initial?.baseUrl || "");
  const [includeInAuto, setIncludeInAuto] = useState(initial?.includeInAuto ?? true);
  const [autoPriority, setAutoPriority] = useState(initial?.autoPriority ?? 10);
  const [supportsVision, setSupportsVision] = useState(initial?.supportsVision ?? false);
  const [mediaModes, setMediaModes] = useState<MediaMode[]>(initial?.mediaModes || []);
  const [mediaProtocol, setMediaProtocol] = useState<MediaProtocol>(initial?.mediaProtocol || "");
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const preview = useMemo(() => endpointPreview(provider, baseUrl), [baseUrl, provider]);
    ...
    <div className="fixed inset-0 z-[100] -top-1/2 -translate-y-full flex items-center justify-center  p-4">
      ...
      >
        ...
        <div className="flex shrink-0 items-start justify-between gap-4 px-5 pb-4 pt-5">
          <div>
            <h2 className="text-[16px] font-semibold">{initial ? "修改模型" : "添加模型"}</h2>
            <p className="mt-1 text-[11px] text-[var(--text-tertiary)]">
              model 值会原样保存到 SQLite，并原样发送给供应商，不会被程序改名。
            </p>
          </div>
          <button type="button" onClick={onClose} className="text-lg" aria-label="关闭添加模型弹窗">
            ×
          </button>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="text-[11px]">
              <span className="mb-1.5 block text-[var(--text-secondary)]">显示名称</span>
              <input
                required
                value={name}
            ...
            <label className="text-[11px]">
              <span className="mb-1.5 block text-[var(--text-secondary)]">供应商 / Key 来源</span>
              <select
                value={provider}
                onChange={(event) => setProvider(event.target.value as LlmProviderId)}
          ...
          <label className="mt-4 block text-[11px]">
            <span className="mb-1.5 block text-[var(--text-secondary)]">模型 model 值</span>
            <input
              required
              value={model}
          ...
          <label className="mt-4 block text-[11px]">
            <span className="mb-1.5 block text-[var(--text-secondary)]">Base URL（可选）</span>
            <input
              value={baseUrl}
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              {MEDIA_MODE_OPTIONS.map((option) => {
                ...
                return (
                  <label key={option.id} className="flex items-center gap-1.5 text-[11px]">
                    <input
                      type="checkbox"
                      checked={checked}
            ...
            {mediaModes.length > 0 && (
              <label className="block text-[11px]">
                ...
                <select
                  value={mediaProtocol}
                  onChange={(event) => setMediaProtocol(event.target.value as MediaProtocol)}
                  className="h-10 w-full rounded-xl border bg-[var(--glass-solid)] px-3 outline-none"
                  style={{ borderColor: "var(--border)" }}
                >
          ...
          >
            ...
            <label className="flex items-center gap-2 text-[11px] sm:col-span-2">
              ...
              <input
                ...
                max={9999}
                value={autoPriority}
                onChange={(event) => setAutoPriority(Number(event.target.value) || 10)}
                className="h-8 w-24 rounded-lg border bg-transparent px-2 outline-none"
                style={{ borderColor: "var(--border)" }}
              />
              <span className="text-[var(--text-tertiary)]">数字越小越先尝试</span>
            </label>
          </div>
