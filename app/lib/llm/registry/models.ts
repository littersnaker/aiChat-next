...
import type { LlmModelDefinition, LlmProviderId } from "../types";
...
export const LLM_MODEL_CATALOG = chatModelConfig.models as unknown as readonly LlmModelDefinition[];

/** 旧版保存值到新版稳定逻辑 ID 的迁移表。 */
const MODEL_ID_ALIASES: Readonly<Record<string, string>> = chatModelConfig.aliases as Readonly<
...
export function getModelDefinition(
  ...
  return LLM_MODEL_CATALOG.find((item) => item.id === normalized || item.model === normalized);
}

/** 返回某供应商的所有聊天模型。 */
export function getModelsForProvider(provider: LlmProviderId): readonly LlmModelDefinition[] {
