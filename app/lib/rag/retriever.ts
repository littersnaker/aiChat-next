...
import type { RagChunk, RagDocumentIndex, RagSearchResult } from "./types";
 ...
 * 为一组切片创建可重复使用的词法索引。
 */
export function createRagDocumentIndex(chunks: readonly RagChunk[]): RagDocumentIndex {
  ...
  return {
    chunks,
    averageTokenCount: chunks.length > 0 ? totalTokenCount / chunks.length : 1,
    documentFrequency,
  };
}
 ...
 */
function scoreTerm(term: string, chunk: RagChunk, index: RagDocumentIndex): number {
  ...
  const matchingDocumentCount = index.documentFrequency.get(term) ?? 0;

  const inverseDocumentFrequency = Math.log(
    1 + (documentCount - matchingDocumentCount + 0.5) / (matchingDocumentCount + 0.5),
  );

  const lengthNormalization =
    1 - BM25_B + BM25_B * (chunk.tokenCount / Math.max(index.averageTokenCount, 1));
...
export function retrieveRagChunks(
  ...
  const scored = index.chunks.map((chunk) => {
    const lexicalScore = uniqueTerms.reduce((sum, term) => sum + scoreTerm(term, chunk, index), 0);
     ...
     */
    const phraseBoost =
      normalizedQuery.length >= 2 && normalizeRagText(chunk.text).includes(normalizedQuery) ? 2 : 0;
