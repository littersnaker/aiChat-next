...
export function useCommerceAgentActions(setAgents: AgentStateSetter) {
  const beginCommerceRun = useCallback(
    (mode: CommerceWorkflowMode = "research") => {
      const now = Date.now();
      setAgents(
        ...
        current.map((agent: AgentInstance) => {
          if (agent.type === "orchestrator") {
            return {
              ...
              currentTask: finished ? "跨境市场情报研究已完成" : `协调：${event.detail}`,
              updatedAt: now,
            };
          }
  ...
  const failCommerceRun = useCallback(
    (message: string) => {
      ...
      setAgents((current: AgentInstance[]) =>
        current.map((agent: AgentInstance) => {
          const isPrimary = agent.type === "orchestrator" || agent.type === "commerce";
          const isActiveResearcher =
            agent.type === "researcher" && ["running", "thinking"].includes(agent.status);
